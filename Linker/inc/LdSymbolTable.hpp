...
class SymbolTable{
  ...
  public:
    //constructor
    SymbolTable();
    SymbolTable(const bool isGlobal);
    ...
    std::vector<std::string> getSymbolsOfType(const SymbolType &wantedType); //must go through reloc table and check global/local stuff   MAYBE NEEDS TO RETURN SYMBOLDATA ALSO
    //isDefined
    bool getSymbolIsDefined(const std::string &label);
    void setSymbolIsDefined(const std::string &label, const bool newIsDefined);
